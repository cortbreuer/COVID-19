...
USCaseCurve <- ggplot(data = USA, mapping = aes(x = DateRep, y = cumsum(Cases))) + geom_line()
...
globalCaseCurve <- ggplot(data = caseSum, mapping = aes(x = DateRep, y = totalCases, group = GeoId, color = GeoId)) + 
  geom_line() + gghighlight(GeoId == c('US', 'ES', 'IT'))

recentCaseSum <- filter(caseSum, Month == 3)
MarchCaseCurve <- ggplot(data = recentCaseSum, mapping = aes(x = DateRep, y = totalCases, group = GeoId)) + geom_line()
...
case100Plot <- ggplot(data = normalizedDate, mapping = aes(x = timeSince, y = totalCases, group = GeoId, color = GeoId)) + 
  geom_line() + 
  gghighlight(GeoId == c('US', 'ES', 'IT')) + 
  scale_y_log10(breaks = c(100, 1000, 10000, 100000)) + 
  annotation_logticks(sides="l") + 
  xlim(0, 30) + 
  theme_bw()

ggsave(plot = case100Plot, here("figures", "2020-03-23_CasePlot.png"), width = 10, height = 7)

#Generate constant growth vectors
day <- seq(0, 20, 1)
third <- data.frame(day = day, growth = (100*(4/3)^day))

case100Plot_thirdGrowth <- ggplot() + geom_line(data = normalizedDate, mapping = aes(x = timeSince, y = totalCases, group = GeoId, color = GeoId)) + 
  geom_point(data = normalizedDate, mapping = aes(x = timeSince, y = totalCases, group = GeoId, color = GeoId)) + 
  gghighlight(GeoId == c('US', 'ES', 'IT')) + 
  scale_y_log10(breaks = c(100, 1000, 10000, 100000)) + 
  xlim(0, 30) + 
  annotation_logticks(sides="l") + xlim(0, 30) + 
  geom_line(data = third, mapping = aes(x = day, y = growth)) + 
  theme_bw()
